...
Public Class Form20
    ...
    Sub total_inf()
        ...
        If (Me.TextBox1.Text.ToString) <> " " Then
            ...
            Dim s11 As String = "SELECT top 1 [no_i] , cast(SUBSTRING([no_i],1,4) as bigint), cast(SUBSTRING([no_i],5,len([no_i])) as bigint) FROM RcvMain where (ISNUMERIC(SUBSTRING([no_i],5,len([no_i])))=1 and cast(SUBSTRING([no_i],1,4) as bigint)=" & Me.dtYear.Value.Year & " and cast(SUBSTRING([no_i],5,len([no_i])) as bigint)=" & Me.TextBox1.Text.Trim() & ") "
            ...
            ds2.Clear()
            Dim sql1 As String
            sql1 = "SELECT no_c as[رقم الصنف],name_snc as[اسم الصنف],qun_r as[الكمية], sal_s as[سعر الوحدة], gema as[القيمة]from View_sl  WHERE no_i='" + TextBox2.Text + "' ORDER BY no_ct1 "
            ...
            If cn.State = ConnectionState.Closed Then
                cn.Open()
            End If
            Dim sql As String = "select * from RcvSub where no_i ='" + TextBox2.Text + "'"
