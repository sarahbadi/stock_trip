...
Public Class C_Form
    ...
    Sub ser_amr()
        ...
        If (Me.TextBox1.Text.ToString) <> "" Then
            ...
            Dim s11 As String = "SELECT top 1 [no_s] , cast(SUBSTRING([no_s],1,4) as bigint), cast(SUBSTRING([no_s],5,len([no_s])) as bigint) FROM IsuMain where (ISNUMERIC(SUBSTRING([no_s],5,len([no_s])))=1 and cast(SUBSTRING([no_s],1,4) as bigint)=" & Me.dtYear.Value.Year & " and no_kshf =" & ssf_t & " and cast(SUBSTRING([no_s],5,len([no_s])) as bigint)=" & Me.TextBox1.Text.Trim() & ") "
